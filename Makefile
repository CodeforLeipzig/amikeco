...
define CMDS
ifeq ($(1), runserver)
	envdir envs/$(ENV) amikeco/manage.py$(1)$(PORT)
else
$(1):
	envdir envs/$(ENV) amikeco/manage.py$(1)
endif
endef
...
create-db:
	envdir envs/$(ENV) createdb -U amikeco -l en_US.utf-8 -E utf-8 -O amikeco -T template0 -e amikeco

create-db-user:
	psql -d postgres -c "CREATE USER \"amikeco\" WITH PASSWORD 'amikeco' CREATEDB;"

drop-db:
	envdir envs/$(ENV) dropdb -i -e -U amikeco amikeco

drop-db-user:
	dropuser -i -e amikeco
...
isort:
	isort --recursive setup.py amikeco/ tests/
...
startapp:
	...
	mkdir -p amikeco/apps/$$app_name; \
	envdir envs/$(ENV) python manage.py startapp $$app_name amikeco/apps/$$app_name --template amikeco/config/app_template; \
	echo "Don't forget to add 'amikeco.apps."$$app_name".apps."$$app_name_title"Config' to INSTALLED_APPS in 'amikeco.config/settings/common.py'!"
...
test-upload:
	...
	python -c "import webbrowser; webbrowser.open('https://testpypi.python.org/pypi/amikeco')"

upload:
	twine upload -s dist/*
	python -c "import webbrowser; webbrowser.open('https://pypi.python.org/pypi/amikeco')"
